import { AutoTokenizer, CLIPTextModelWithProjection, AutoProcessor, CLIPVisionModelWithProjection, Tensor, RawImage } from "@xenova/transformers";
...
let tokenizerInstance = await AutoTokenizer.from_pretrained(MODEL_NAME);
let textModelInstance = await CLIPTextModelWithProjection.from_pretrained(MODEL_NAME);
let processorInstance = await AutoProcessor.from_pretrained(MODEL_NAME);
let visionModelInstance = await CLIPVisionModelWithProjection.from_pretrained(MODEL_NAME);
...
async function loadModelAndResources(): Promise<void> {
  if (!tokenizerInstance || !textModelInstance || !processorInstance || !visionModelInstance) {
    ...
    console.log("Model and resources loaded");
  }
}
